...
export function SessionRow({ row, primary }: { row: Row; primary?: boolean }) {
  ...
  if (primary) {
    return (
      <Link
        href={`/session/${row.id}`}
        className={
          'block rounded-2xl px-5 py-4 text-white dark:text-black ' +
  ...
  return (
    <Link
      href={`/session/${row.id}`}
      className="flex items-baseline justify-between gap-3 rounded-xl px-1 py-2 hover:bg-black/5 dark:hover:bg-white/10"
    >
