...
export const templateItems = pgTable(
  'template_item',
  {
    ...
    /**
     * Подводка: веса ступеней по возрастанию, в килограммах.
     * Пусто — упражнение начинается сразу с рабочего веса.
     *
     * Килограммами, а не долями рабочего веса: доли ехали вместе с ним, но
     * на экране превращались в 66.7 и 88.9, и человек переставал узнавать
     * то, что сам вводил. Штанга собирается из блинов, а не из процентов.
     * Ступени тяжелее рабочего веса движок отбрасывает — после отката
     * за паузу подводка сама укорачивается.
     */
    leadKg: numeric('lead_kg', { precision: 7, scale: 2, mode: 'number' }).array(),
    note: text('note'),
  },
