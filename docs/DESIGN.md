...
переносится между залами.
    ...
    template_item(
      ...
      rep_max               int not null default 12,  -- он же целевые повторы
      note                  text
    )
...
**Целевые повторы вводятся одним числом.** В интерфейсе одно поле — столько,
сколько собираешься сделать; оно становится `rep_max`. Нижняя граница выводится
как `rep_max − 2` и наружу не показывается.

Диапазон нужен движку, а не человеку: пока повторы растут внутри него, вес стоит,
а закрыл верх — прибавился. Без буфера прогрессия становится линейной, то есть
каждый раз либо плюс вес, либо ничего, и потолок приходит быстро. Но держать
в голове два числа человек не обязан, поэтому нижняя граница живёт внутри
и всплывает только в пояснении, когда вес снижается.
